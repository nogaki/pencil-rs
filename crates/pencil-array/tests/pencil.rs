...
fn expected_range(global_len: usize, coordinate: usize, process_count: usize) -> Range<usize> {
    global_len * coordinate / process_count
        ..global_len * (coordinate + 1) / process_count
}
...
fn row_major_defaults_and_decomposition_order_define_local_ranges() {
    ...
    assert_eq!(
        default.decomposition().map(SpatialAxis::index),
        [0, 1]
    );
    ...
    assert_eq!(
        ...
        &[
            expected_range(10, c0, 2),
            expected_range(14, c1, 2),
            0..6,
        ]
    );
    ...
    assert_eq!(
        ...
        &[
            expected_range(10, c1, 2),
            expected_range(14, c0, 2),
            0..6,
        ]
    );
