...
Milestone 9 is an opt-in checker for the distributed `pencil-fft` C2C
forward/inverse/backward and R2C/C2R APIs. C2C backward is the positive-sign,
unnormalized route from reversed output layout to canonical input layout. It
changes no production tolerances, CI, or checked-in numeric data.
...
6 MPI ranks. C2C fixtures additionally contain raw `backward_expected` values;
R2C fixtures retain their existing four sections. It verifies a test-run marker
so a missing ignored test cannot pass silently. It also runs independent
private pristine-fixture copies with `forward_expected` and
`backward_expected` corruption; each must fail at one rank with a comparison
error marker, and the backward check must include `OOP backward` context.
Missing Julia, MPI, fixtures, or matrix members is a failure, never a skip. The
runner uses Cargo's selected toolchain and target
...
section backward_expected complex COUNT
... COUNT values ...
end
```

C2C files have five counted sections in this order, with
`backward_expected` generated by FFTW's unnormalized
`bfft(copy(inverse_input), spatial_dims)`. R2C files retain exactly four
sections and do not accept a backward section. Real values are one
...
spectrum from a real input, snapshots it before planning, and executes C2R on a
private copy. FFTW `ESTIMATE`, one thread, the `fftw`
...
spectrum preservation, normalized inverse, raw backward, and C2C in-place
values.
