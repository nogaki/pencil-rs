...
backward is the corresponding raw C2R route. It
...
error marker, and the backward check must include `R2C backward` context.
...
Every fixture has five counted sections in this order. C2C
`backward_expected` is generated by FFTW's unnormalized
...
R2C comparison uses that raw result directly. Real values are one finite number
and are represented internally as a zero-imaginary complex value; complex values
are two finite numbers. Rust derives the reduced output shape and topology grids
and uses fixed precision-specific comparison bounds. The generator fixes
`ESTIMATE` and one FFTW thread; the Rust checker runs both methods. These
constants are not serialized repeatedly.
